...
for i in $(seq 1 $iter_num); do
    ...
    ACCELERATE_LOG_LEVEL=info accelerate launch E_step_ent.py \
    ...
    python inference.py --model_path "${e_model_dir}/final_checkpoint" --task_type "${task_pre}_${task_suf}" --dataset_path $dataset_path --iter $i
    conda deactivate

    conda activate sft_debug
    accelerate launch m_sft.py --deepspeed deepspeed_configs/deepspeed_2.json --model_name $e_input_model --attn_implementation eager --per_device_train_batch_size 4 --gradient_accumulation_steps 2 --train_set_path $dataset_path --output_dir $m_model_dir --hub_model_id $m_hub_id
    conda deactivate
done